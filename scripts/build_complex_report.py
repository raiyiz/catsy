"""Build a commit-addressed, interactive static report for the complex example."""
...
def build_run_page(site_run_root: Path, commit: str) -> None:
    ...
    journals = sorted(path for path in site_run_root.rglob("*") if path.suffix.lower() in {".json", ".jsonl"})
    ...
    for plot in plots:
        ...
        gallery.append(
            f'''<button class="gallery-card" data-src="{esc(relative)}" data-title="{esc(title)}" type="button">'''
            f'''<span class="gallery-image"><img src="{esc(relative)}" alt="{esc(title)}" loading="lazy"></span>'''
            f'''<span class="gallery-caption"><strong>{esc(title)}</strong><small>inspect ↗</small></span></button>'''
        )
    ...
    for number, title, description, plot_stem, category in STAGES:
        ...
        if plot:
            relative = plot.relative_to(site_run_root).as_posix()
            visual = (
                f'<button class="stage-image" data-src="{esc(relative)}" data-title="{esc(title)}" type="button">'
                f'<img src="{esc(relative)}" alt="{esc(title)}" loading="lazy"></button>'
            )
        ...
        stage_cards.append(
            ...
            f'<div class="stage-index">{esc(number)}</div>{visual}'
            f'<div class="stage-body"><div class="stage-kicker">{esc(category)}</div><h3>{esc(title)}</h3><p>{esc(description)}</p></div></article>'
        )
    ...
    for journal in journals:
        ...
        journal_links.append(
            f'<a class="journal-file" href="{esc(relative)}" target="_blank" rel="noopener">'
            f'<span class="file-icon">{esc(journal.suffix[1:].upper())}</span>'
            f'<span><strong>{esc(relative)}</strong><small>{journal.stat().st_size:,} bytes · open raw file ↗</small></span></a>'
        )
...
<meta name="description" content="Interactive Catsy complex experiment report for {esc(short_commit)}">
...
<body>
<header class="topbar"><div class="container topbar-inner"><a class="brand" href="../..">catsy <span>LAB</span></a><nav class="nav"><a href="../../">all runs</a><a href="#pipeline">pipeline</a><a href="#stages">explorer</a><a href="#gallery">plots</a><a href="#journal">journal</a></nav></div></header>
<main>
...
<div class="badges"><span class="badge good">● CI generated</span><span class="badge">commit <code>{esc(short_commit)}</code></span><span class="badge">{plot_count} diagnostics</span><span class="badge">{journal_count} journal files</span></div>
<div class="meta-grid"><div class="metric"><div class="value">3</div><div class="label">Gaussian modes</div></div><div class="metric"><div class="value">2</div><div class="label">measurement schemes</div></div><div class="metric"><div class="value">33</div><div class="label">MZI phase points</div></div><div class="metric"><div class="value">{plot_count}</div><div class="label">saved diagnostics</div></div></div>
<div class="links" style="margin-top:20px"><a class="button" href="https://github.com/raiyiz/catsy/commit/{esc(commit)}" target="_blank" rel="noopener">view commit ↗</a><a class="button" href="https://github.com/raiyiz/catsy/actions" target="_blank" rel="noopener">view CI ↗</a><a class="button" href="../../">← all runs</a></div></div></section>
<section class="section alt" id="pipeline"><div class="container"><div class="section-head"><div><div class="eyebrow">The calculation</div><h2>One experiment, eleven moments</h2><p>Follow the state as it crosses representations, operations, interferometry, and measurement.</p></div></div><div class="pipeline">{"".join(f'<a class="pipeline-step" href="#stage-{n}"><span class="num">{esc(n)}</span><strong>{esc(t)}</strong><small>{esc(c)}</small></a>' for n,t,d,p,c in STAGES)}</div></div></section>
<section class="section" id="stages"><div class="container"><div class="section-head"><div><div class="eyebrow">Simulation explorer</div><h2>Explore the experiment</h2><p>Filter the physical layer you care about, then open any diagnostic at full resolution.</p></div></div><div class="filters" role="toolbar" aria-label="Filter experiment stages"><button class="filter active" data-filter="all" type="button">all stages</button><button class="filter" data-filter="gaussian" type="button">Gaussian</button><button class="filter" data-filter="fock" type="button">Fock space</button><button class="filter" data-filter="interferometer" type="button">interferometer</button><button class="filter" data-filter="measurement" type="button">measurements</button></div><div class="stages">{"".join(stage_cards)}</div></div></section>
<section class="section alt" id="gallery"><div class="container"><div class="section-head"><div><div class="eyebrow">Diagnostics</div><h2>Visual gallery</h2><p>Click a plot to inspect it in the built-in viewer. No download required.</p></div></div><div class="gallery">{"".join(gallery) or '<p>No plots were generated.</p>'}</div></div></section>
<section class="section" id="journal"><div class="container"><div class="section-head"><div><div class="eyebrow">Reproducibility</div><h2>Experiment journal</h2><p>The raw journal lives beside the plots so the visual record and machine-readable record stay together.</p></div></div><div class="journal">{"".join(journal_links) or '<p>No journal files were generated.</p>'}</div></div></section>
</main>
...
<footer class="footer"><div class="container"><strong>Catsy Lab</strong> · complex example · commit <code>{esc(commit)}</code><br><span>Static report generated by CI; visualizations are produced exclusively through Catsy's plotting helpers.</span></div></footer>
<script>
(function(){{
