...
def _render_stages(stages: list[dict[str, object]], run_root: Path) -> str:
    ...
    for number, stage in enumerate(stages, start=1):
        ...
        if plot_stem:
            plot = next(
                (p for p in plots_root.glob("*.png") if p.stem == str(plot_stem)),
                None,
            )
        ...
        cards.append(
            f'<article class="stage" data-category="{esc(category)}" '
            f'id="stage-{number:02d}">'
...
def sync(report_path: Path, journal_path: Path, run_root: Path) -> None:
    ...
    text = text.replace(
        ...
        "<p>Stages are read from the execution journal, so this map follows "
        "what the simulation actually ran.</p>",
        1,
    )
...
def main() -> None:
    ...
    sync(report_path, _find_journal(site_run_root), site_run_root)
