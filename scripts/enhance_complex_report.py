...
INSIGHTS = {
    "01": ("State preparation", "The phase-space view shows the prepared signal mode before non-Gaussian processing."),
    "02": ("Covariance structure", "Read the diagonal as single-mode variances and the off-diagonal blocks as inter-mode correlations."),
    "03": ("Correlation structure", "This diagnostic makes the Gaussian coupling visible: correlated quadratures indicate information shared between modes."),
    "04": ("Non-Gaussian signature", "The Wigner function should reveal the two-lobed cat structure and its interference fringes."),
    "05": ("Fock-space evidence", "Use the photon-number structure to see how the even-parity cat is represented beyond a Gaussian description."),
    "06": ("Heralded subtraction", "Loss and detector efficiency make this a realistic conditional operation rather than an ideal annihilation."),
    "07": ("Heralded addition", "Compare this state with subtraction to see how conditional photon-number engineering changes the non-Gaussian state."),
    "08": ("Interference response", "The 33-point phase scan probes how the processed state responds to a Mach–Zehnder phase shift."),
    "09": ("Quadrature conditioning", "Homodyne selects one measured quadrature, leaving a conditional idler state whose phase-space structure can be inspected."),
    "10": ("Complex-amplitude conditioning", "Heterodyne samples both quadratures simultaneously, giving a two-dimensional measurement outcome."),
    "11": ("Readout comparison", "Compare homodyne and heterodyne conditioning to see how the measurement model changes the inferred state."),
}
...
def refine_run(run_root: Path, timestamp: str) -> None:
    ...
    text = re.sub(r'\s*<section class="section alt" id="gallery">.*?</section>', "", text, flags=re.S)
    ...
    timestamp_label = datetime.fromisoformat(timestamp.replace("Z", "+00:00")).strftime("%Y-%m-%d %H:%M UTC")
    text = text.replace(
...
def rebuild_archive(runs_root: Path) -> None:
    cards = []
    run_dirs = sorted((p for p in runs_root.iterdir() if p.is_dir()), key=lambda p: read_timestamp(p), reverse=True)
    for run in run_dirs:
        commit = run.name
