...
class TestEvolutionVisualizations:
    ...
    def test_evolution_dashboard_renders_expected_panels(
    ...
    @pytest.mark.visualize
    def test_multimode_evolution_renders_expected_panels(
        self, assert_no_empty_axes, assert_layout_can_render
    ) -> None:
        """plot_multimode_evolution's own contract: one phase-space panel per
        mode plus a shared cross-mode correlation panel. The richer,
        hand-built TMSV-under-a-Hamiltonian story built from the same
        _tmsv_hamiltonian_evolution() fixture lives in the showcase gallery
        instead -- it doesn't exercise this function, so testing it here
        would leave plot_multimode_evolution's actual contract unchecked.
        """
        states, times = _tmsv_hamiltonian_evolution()
        figure = plot_multimode_evolution(states, times=times)

        assert len(figure.axes) == 3  # one per mode ("a", "b") + correlation
        assert figure._suptitle is not None
        assert "a" in figure._suptitle.get_text()
        assert "b" in figure._suptitle.get_text()
        correlation_ax = figure.axes[-1]
        assert correlation_ax.get_xlabel() == "time"
        assert len(correlation_ax.lines) == 1
    ...
    @pytest.mark.visualize
    def test_timecoded_phase_space_trajectory_renders_expected_panels(
