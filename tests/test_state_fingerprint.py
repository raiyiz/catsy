...
def _fingerprint_rows(figure):
    table = next(
        artist for artist in figure.axes[0].get_children() if artist.__class__.__name__ == "Table"
    )
    return {
        str(row.get_text()).strip(): str(value.get_text()).strip()
        for (row, value) in table.get_celld().values()
        if hasattr(row, "get_text") and hasattr(value, "get_text")
    }
...
def test_state_fingerprint_matches_state_invariants() -> None:
    ...
    displayed = {
        ...
        for row in range(1, 5 + len(state.modes))
...
def test_state_fingerprint_uses_single_structural_panel() -> None:
    figure = plot_state_fingerprint(GaussianState.coherent(("a", "b"), [0.4 + 0.1j, -0.2 + 0.3j]))
    figure.canvas.draw()
