...
def _set_phase_limits(
    ...
    x = max(_ellipse_extents(mean, covariance, n_sigma)[0] for mean, covariance in zip(means, covariances))
    p = max(_ellipse_extents(mean, covariance, n_sigma)[1] for mean, covariance in zip(means, covariances))
...
def _style_phase_axes(ax: plt.Axes) -> None:
    ax.axhline(0.0, linewidth=0.6, linestyle="--", alpha=0.30, zorder=0)
    ax.axvline(0.0, linewidth=0.6, linestyle="--", alpha=0.30, zorder=0)
...
def plot_phase_space(
    ...
    *,
    ...
    if ax is None:
        ...
        fig = cast(plt.Figure, ax.figure)
    _add_ellipse(ax, mean, covariance, n_sigma, alpha=0.55, label=fr"{n_sigma:g}$\sigma$")
    values, vectors = np.linalg.eigh(covariance)
    order = np.argsort(values)[::-1]
...
def plot_phase_space_trajectory(
    ...
    *,
    ...
    for index in indices:
        _add_ellipse(ax, means[index], covariances[index], n_sigma, alpha=0.25)
    _style_phase_axes(ax)
    ...
    *,
    ...
    def update(frame: int) -> tuple[object, ...]:
        ...
        for index, line in enumerate((major_axis, minor_axis)):
            ...
            length = n_sigma * np.sqrt(float(values[index]))
            line.set_data(
                [mean[0] - vector[0] * length, mean[0] + vector[0] * length],
                [mean[1] - vector[1] * length, mean[1] + vector[1] * length],
            )
...
def plot_covariance_evolution(
    ...
    *,
    ...
    """Plot the independent entries of a single-mode covariance over time."""
    sequence = _states(states)
    if times is not None and len(times) != len(sequence):
        raise ValueError("times must have the same length as states.")
    ...
    ax.axhline(0.0, linewidth=0.6, linestyle="--", alpha=0.3)
...
def plot_diagnostics(
    ...
    *,
    ...
    ax.plot(x, minimum_nu, label=r"minimum symplectic eigenvalue $\nu$")
    ax.set_xlabel("time" if times is not None else "step")
    ax.set_ylabel("value")
...
def plot_wigner(
    ...
    *,
    ...
    """Plot the analytical single-mode Wigner function."""
    if x_max <= 0:
        raise ValueError("x_max must be positive.")
    if num_points < 2:
        raise ValueError("num_points must be at least 2.")
    X, P, wigner = _wigner_grid(state, mode_name, x_max=x_max, num_points=num_points)
    if ax is None:
...
def plot_wigner_evolution(
    ...
    *,
    ...
    """Plot Wigner snapshots with a shared spatial and intensity scale."""
    sequence = _states(states)
    if times is not None and len(times) != len(sequence):
        raise ValueError("times must have the same length as states.")
    ...
    fig, axes = plt.subplots(1, len(selected), figsize=(4.6 * len(selected), 4.5), squeeze=False)
    axes_flat = axes[0]
    image = None
    for position, (index, (X, P, wigner)) in enumerate(zip(selected, grids)):
...
def plot_evolution(
    ...
    *,
    ...
    """Create a publication-oriented dashboard for Gaussian evolution."""
    sequence = _states(states)
    if n_sigma <= 0:
        raise ValueError("n_sigma must be positive.")
    if times is not None and len(times) != len(sequence):
    ...
    ax_wigner = fig.add_subplot(grid[1, 0])
    ...
    snapshot = selected[-1]
    plot_wigner(sequence[snapshot], mode_name, ax=ax_wigner)
    ax_wigner.set_title(
        f"Wigner snapshot — {('t = ' + format(times[snapshot], 'g')) if times is not None else ('step ' + str(snapshot))}",
        pad=14,
        fontweight="medium",
    )
