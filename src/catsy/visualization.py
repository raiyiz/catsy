...
def _state_label(state: GaussianState, mode_name: str | None = None) -> str:
...
def _add_ellipse(ax: plt.Axes, mean: np.ndarray, covariance: np.ndarray, n_sigma: float, **kwargs: object) -> Ellipse:
    width, height, angle = _ellipse_geometry(covariance, n_sigma)
    ellipse = Ellipse(xy=(float(mean[0]), float(mean[1])), width=width, height=height, angle=angle, fill=False, linewidth=1.8, **kwargs)
    ax.add_patch(ellipse)
    return ellipse


def _set_phase_space_limits(ax: plt.Axes, means: np.ndarray, covariances: Sequence[np.ndarray], n_sigma: float, *, padding: float = 0.18) -> None:
    ...
    extent = max(x_extent, p_extent, 1.0) * (1.0 + padding)
...
def plot_covariance_matrix(state: GaussianState, *, ax: plt.Axes | None = None, annotate: bool = True, show: bool = False) -> plt.Figure:
    ...
    ax.set_title("Covariance matrix", pad=14)
    ax.set_xlabel("quadrature", labelpad=8)
    ax.set_ylabel("quadrature", labelpad=8)
    fig.colorbar(image, ax=ax, fraction=0.046, pad=0.04, label="V")
    ...
    if annotate:
        ...
        for row in range(covariance.shape[0]):
            for col in range(covariance.shape[1]):
                ...
                ax.text(col, row, f"{value:.2f}", ha="center", va="center", color="white" if abs(value) > threshold else "black", fontsize=9)
...
def plot_phase_space(state: GaussianState, mode_name: str, *, ax: plt.Axes | None = None, n_sigma: float = 2.0, show: bool = False) -> plt.Figure:
    ...
    if ax is None:
        ...
        fig = cast(plt.Figure, ax.figure)
    _add_ellipse(ax, mean, covariance, n_sigma, label=f"{n_sigma:g}σ uncertainty")
    eigenvalues, eigenvectors = np.linalg.eigh(covariance)
    order = np.argsort(eigenvalues)[::-1]
    for eigenvalue, vector in zip(eigenvalues[order], eigenvectors[:, order].T):
        length = n_sigma * np.sqrt(max(float(eigenvalue), 0.0))
        ax.plot([mean[0] - vector[0] * length, mean[0] + vector[0] * length], [mean[1] - vector[1] * length, mean[1] + vector[1] * length], linewidth=1.0, alpha=0.45)
    ax.scatter([mean[0]], [mean[1]], s=55, zorder=3, label="mean")
    ax.axhline(0.0, linewidth=0.7, linestyle="--", alpha=0.45)
    ax.axvline(0.0, linewidth=0.7, linestyle="--", alpha=0.45)
    ax.set_xlabel("x quadrature")
    ax.set_ylabel("p quadrature")
    ax.set_title("Phase space", pad=14)
...
def plot_phase_space_trajectory(states: Sequence[GaussianState], mode_name: str, *, times: Sequence[float] | None = None, ellipse_every: int | None = None, n_sigma: float = 2.0, ax: plt.Axes | None = None, show: bool = False) -> plt.Figure:
    ...
    if ax is None:
        ...
        fig = cast(plt.Figure, ax.figure)
    ax.plot(means[:, 0], means[:, 1], linewidth=2.0, label="mean trajectory")
    ax.scatter([means[0, 0]], [means[0, 1]], s=45, label="initial", zorder=3)
    ax.scatter([means[-1, 0]], [means[-1, 1]], s=60, marker="*", label="final", zorder=3)
    ...
    for index in indices:
        _add_ellipse(ax, means[index], covariances[index], n_sigma, alpha=0.3)
    ax.set_aspect("equal", adjustable="box")
    ax.set_xlabel("x quadrature")
    ax.set_ylabel("p quadrature")
    ax.set_title("Phase-space evolution", pad=14)
...
def animate_phase_space(states: Sequence[GaussianState], mode_name: str, *, times: Sequence[float] | None = None, n_sigma: float = 2.0, interval: int = 80, repeat: bool = False, ax: plt.Axes | None = None, show: bool = False) -> FuncAnimation:
    ...
    if ax is None:
        ...
        fig = cast(plt.Figure, ax.figure)
    ax.plot(means[:, 0], means[:, 1], linestyle="--", linewidth=0.9, alpha=0.25, label="full trajectory")
    ax.scatter([means[0, 0]], [means[0, 1]], marker="o", s=45, zorder=4, label="initial")
    _set_phase_space_limits(ax, means, covariances, n_sigma)
    ax.set_aspect("equal", adjustable="box")
    ax.set_xlabel("x quadrature")
    ax.set_ylabel("p quadrature")
    ax.set_title("Gaussian phase-space dynamics", pad=14)
    ...
    time_text = ax.text(0.03, 0.91, "", transform=ax.transAxes, va="top", fontsize=11)
    ...
    def update(frame: int) -> tuple[object, ...]:
        ...
        for index, line in enumerate((major_axis, minor_axis)):
            ...
            line.set_data([mean[0] - vector[0] * length, mean[0] + vector[0] * length], [mean[1] - vector[1] * length, mean[1] + vector[1] * length])
        if times is None:
            time_text.set_text(f"step {frame + 1} / {len(sequence)}")
        else:
            time_text.set_text(f"t = {times[frame]:g}")
        stats_text.set_text(f"σ₁ = {np.sqrt(eigenvalues[0]):.3g}    σ₂ = {np.sqrt(eigenvalues[1]):.3g}    det(V) = {np.linalg.det(covariance):.3g}")
        return point, trail, ellipse, major_axis, minor_axis, time_text, stats_text

    animation = FuncAnimation(fig, update, frames=len(sequence), interval=interval, blit=False, repeat=repeat)
    update(0)
    if show:
        plt.show()
    return animation
...
def plot_covariance_evolution(states: Sequence[GaussianState], mode_name: str, *, times: Sequence[float] | None = None, ax: plt.Axes | None = None, show: bool = False) -> plt.Figure:
    ...
    ax.set_title("Covariance evolution", pad=14)
    _state_header(ax, sequence[-1], mode_name)
...
def plot_diagnostics(states: Sequence[GaussianState], *, times: Sequence[float] | None = None, ax: plt.Axes | None = None, show: bool = False) -> plt.Figure:
    ...
    ax.set_xlabel("time" if times is not None else "step")
    ax.set_ylabel("value")
    ax.set_title("State diagnostics", pad=14)
    _state_header(ax, sequence[-1])
    ax.legend(frameon=False, loc="best")
    return _finalize(fig, show)


def plot_wigner(state: GaussianState, mode_name: str, *, x_max: float = 4.0, num_points: int = 180, ax: plt.Axes | None = None, show: bool = False) -> plt.Figure:
    ...
    ax.set_xlabel("x quadrature")
    ax.set_ylabel("p quadrature")
    ax.set_title("Wigner function", pad=14)
    _state_header(ax, state, mode_name)
    fig.colorbar(image, ax=ax, fraction=0.046, pad=0.04, label="W(x, p)")
    return _finalize(fig, show)
...
def plot_wigner_evolution(states: Sequence[GaussianState], mode_name: str, *, times: Sequence[float] | None = None, indices: Sequence[int] | None = None, x_max: float = 4.0, num_points: int = 120, show: bool = False) -> plt.Figure:
    ...
    for position, index in enumerate(selected):
        ...
        axes[0, position].set_title((f"t = {times[index]:g}" if times is not None else f"step {index}"), pad=14)
    return _finalize(fig, show)
...
def plot_evolution(states: Sequence[GaussianState], mode_name: str, *, times: Sequence[float] | None = None, wigner_indices: Sequence[int] | None = None, n_sigma: float = 2.0, show: bool = False) -> plt.Figure:
    ...
    axes[1, 0].set_title("Wigner snapshot", pad=14)
    fig.suptitle(f"Gaussian-state evolution · mode {mode_name}", fontsize=14)
    return _finalize(fig, show)
...
def plot_state_dashboard(state: GaussianState, *, mode: str | None = None, show: bool = False) -> plt.Figure:
    ...
    fig.suptitle(f"Gaussian state · {', '.join(state.modes)}", fontsize=14)
    return _finalize(fig, show)
