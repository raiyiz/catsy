...
`MachZehnderInterferometer` (in #src-link("src/catsy/experiments/mzi.py", line: 23, label: [`mzi.py`])) models a two-mode interferometer in which an input state (e.g. a Schrödinger-cat state plus vacuum in the second port) passes through the sequence
