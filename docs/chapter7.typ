...
`MachZehnderInterferometer` (in #src-link("src/catsy/optics.py", line: 93, label: [`optics.py`])) models a two-mode interferometer in which an input state (e.g. a Schrödinger-cat state plus vacuum in the second port) passes through the sequence
