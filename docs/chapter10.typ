...
#table(
  ...
  [#src-link("src/catsy/core.py")], [`Circuit` (generic executable operation sequence).],
...
*Declarative, via `Circuit` (Chapter 3):* a `Circuit` describes the gate sequence, and `run` executes it against an explicitly supplied initial state.
